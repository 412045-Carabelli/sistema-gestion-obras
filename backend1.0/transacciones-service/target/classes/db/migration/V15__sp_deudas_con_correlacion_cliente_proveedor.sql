-- V15__sp_deudas_con_correlacion_cliente_proveedor.sql
-- Actualiza SPs para correlación bidireccional:
-- - Cuando se filtra por cliente, traer sus obras + proveedores de esas obras
-- - Cuando se filtra por proveedor, traer sus obras + clientes de esas obras

DROP PROCEDURE IF EXISTS sp_deudas_globales_con_grupo;
DROP PROCEDURE IF EXISTS sp_deudas_proveedores_con_grupo;
GO

-- Deudas con CLIENTES (presupuesto - cobrado = saldo)
-- Ahora acepta proveedorId y filtra obras que tengan ese proveedor
...
-- Deudas con PROVEEDORES (presupuestado - pagado = saldo)
-- Ahora acepta clienteId y filtra obras que tengan ese cliente
