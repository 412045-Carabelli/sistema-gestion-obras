-- V26__exclude_demasia_from_cuenta_corriente.sql
-- Excluye tipo_costo = 'DEMASIA' del calculo de costosTotal en sp_dashboard_cuenta_corriente.
-- DEMASIA solo impacta el presupuesto de la obra (obra.presupuesto), no genera
-- obligacion de pago hacia ningun proveedor.
...
BEGIN
  ...
  -- DEMASIA excluido: no genera obligacion de pago a proveedores
  ...
  WHERE oc.activo = 1
    ...
    AND ISNULL(oc.tipo_costo, '') <> 'DEMASIA';
